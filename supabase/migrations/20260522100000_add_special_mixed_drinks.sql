...
set
  ...
  volume = '2cl',
  abv = 37
where regexp_replace(lower(btrim(coalesce(beer_name, ''))), '[^a-z0-9]+', ' ', 'g') in (
  'vodka orange juice',
  'vodka orange'
);
...
as $$
  ...
  select
    ...
    coalesce(round(max(abv_value) filter (where not is_rtd and not is_jagerbomb and not is_sambuca and not is_special_mixed)::numeric, 1)::double precision, 0) as strongest_abv,
...
comment on function public.get_profile_stats(uuid) is 'Profile aggregate stats with session-based trophy day buckets, normalized drink uniqueness, parsed beverage serving volumes, RTD/Jagerbomb/Sambuca/special mixed drink counters, beer-only strongest ABV calculation, and 2+ true-pint weekly streaks.';

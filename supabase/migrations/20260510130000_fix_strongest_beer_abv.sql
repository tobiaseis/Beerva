...
as $$
  ...
  select
    ...
    coalesce(round(max(abv_value) filter (where not is_rtd and not is_jagerbomb and not is_sambuca)::numeric, 1)::double precision, 0) as strongest_abv,
...
comment on function public.get_profile_stats(uuid) is 'Profile aggregate stats with session-based trophy day buckets, normalized drink uniqueness, parsed beverage serving volumes, RTD/Jägerbomb/Sambuca trophy counters, and beer-only strongest ABV calculation.';

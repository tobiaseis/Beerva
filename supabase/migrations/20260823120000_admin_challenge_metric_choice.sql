-- Admins choose whether a challenge counts true pints or Danish alcohol units.
--
-- Until now the metric was derived from the challenge type: every target
-- challenge was forced to alcohol units and every leaderboard challenge
-- defaulted to true pints. The stored metric_type column now decides for both
-- challenge types, so the admin form can offer the choice.

-- Existing target challenges were already migrated to alcohol units by
-- 20260618140000. Repeat the backfill so no live challenge changes meaning when
-- the effective-metric helper stops forcing the target case below.
update public.challenges
set metric_type = 'alcohol_units'
where challenge_type = 'target'
  and metric_type is distinct from 'alcohol_units';

create or replace function public.beerva_effective_challenge_metric_type(
  challenge_type_value text,
  stored_metric_type text
)
returns text
language sql
immutable
...
as $$
  select case
    when nullif(btrim(coalesce(stored_metric_type, '')), '') = 'alcohol_units' then 'alcohol_units'
    else 'true_pints'
  end;
$$;

comment on function public.beerva_effective_challenge_metric_type(text, text) is
  'Resolves the stored challenge metric, defaulting to true pints when it is missing.';

grant execute on function public.beerva_effective_challenge_metric_type(text, text) to authenticated;
...
create or replace function public.admin_save_challenge(
  ...
  challenge_metric_type text default null,
...
declare
  ...
  requested_metric_type text := nullif(btrim(coalesce(challenge_metric_type, '')), '');
  resolved_metric_type text;
...
begin
  ...
  if requested_metric_type is not null
    and requested_metric_type not in ('true_pints', 'alcohol_units') then
    raise exception 'Challenge metric must be true pints or alcohol units.';
  end if;
  ...
  if target_challenge_type = 'target' then
    if challenge_target_value is null or challenge_target_value <= 0 then
      raise exception 'Target % must be greater than 0.',
        case
          when coalesce(requested_metric_type, 'alcohol_units') = 'alcohol_units' then 'units'
          else 'true pints'
        end;
    end if;
  ...
  if target_challenge_id is null then
    -- Older clients that do not send a metric keep the previous defaults.
    resolved_metric_type := coalesce(
      requested_metric_type,
      case when target_challenge_type = 'target' then 'alcohol_units' else 'true_pints' end
    );
  ...
  -- Editing from a client that does not send a metric keeps the stored one.
  resolved_metric_type := coalesce(
    requested_metric_type,
    nullif(btrim(coalesce(existing_row.metric_type, '')), ''),
    case when target_challenge_type = 'target' then 'alcohol_units' else 'true_pints' end
  );
  ...
  if has_entries and (
    ...
    or existing_row.metric_type is distinct from resolved_metric_type
...
-- Winner announcements must name the metric the challenge was actually scored
-- on. `true_pints` stays in the metadata so clients released before this
-- migration keep rendering a value.
...
begin
  ...
  loop
    ...
    award_row_id := null;
    post_row_id := null;
    challenge_metric_type := public.beerva_effective_challenge_metric_type(
      challenge_row.challenge_type,
      challenge_row.metric_type
    );
    challenge_metric_label := case
      when challenge_metric_type = 'alcohol_units' then 'units'
      else 'true pints'
    end;
    ...
    insert into public.official_feed_posts (
      ...
      jsonb_build_object(
        ...
        'metric_type', challenge_metric_type,
        'progress_value', round(leader_row.progress_value::numeric, 1),

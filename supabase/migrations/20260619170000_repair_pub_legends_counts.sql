...
as $$
  ...
  beer_session_totals as (
    select
      ...
      count(session_beers.id)::integer as beer_row_count,
      coalesce(sum(
        public.beerva_serving_volume_ml(session_beers.volume)
        * greatest(coalesce(session_beers.quantity, 1), 0)
        / 568.0
      ) filter (where coalesce(session_beers.excluded_from_stats, false) = false), 0) as true_pints,
      coalesce(sum(
        greatest(coalesce(session_beers.quantity, 1), 0)
      ) filter (where coalesce(session_beers.excluded_from_stats, false) = false), 0)::integer as drink_count
    from published_sessions
    left join public.session_beers on session_beers.session_id = published_sessions.session_id
  ...
  session_totals as (
    select
      published_sessions.*,
      case
        when beer_session_totals.beer_row_count > 0 then beer_session_totals.true_pints
        else coalesce(
      ...
      (
        case
          when beer_session_totals.beer_row_count > 0 then beer_session_totals.drink_count
          else greatest(coalesce(published_sessions.quantity, 1), 0)
        end
      )::integer as drink_count
    from published_sessions
    join beer_session_totals on beer_session_totals.session_id = published_sessions.session_id
  ),
  pub_rollups as (
    select
...
comment on function public.get_pub_legends(integer) is
  'Returns the most visited real pubs from published sessions with live session counts and each pub champion by true pints.';

comment on function public.get_pub_king_of_the_pub(text, integer) is
  'Returns each user''s best published true-pint session at a real pub, ranked King of the Pub style.';

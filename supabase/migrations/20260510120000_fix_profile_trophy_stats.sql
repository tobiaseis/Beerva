...
as $$
  with base as (
    select
      ...
      case lower(btrim(coalesce(session_beers.volume, 'pint')))
...
comment on function public.get_profile_stats(uuid) is 'Profile aggregate stats with session-based trophy day buckets and normalized beer uniqueness.';
